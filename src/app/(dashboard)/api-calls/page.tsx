...
// -----------------------------
// PREDEFINED ENDPOINTS (includes access-key endpoints)
// -----------------------------
const PREDEFINED_ENDPOINTS: PredefinedEndpoint[] = [
  {
    name: 'Ping',
  ...
  {
    name: 'Email • Verify Bulk (JWT)',
    method: 'POST',
    url: '/api/email/verifyBulkEmail',
    description: 'Verify a list of emails (JWT)',
    headers: [
      { id: '1', key: 'Content-Type', value: 'application/json', enabled: true },
      { id: '2', key: 'Authorization', value: 'Bearer ACCESS_TOKEN', enabled: true }
    ],
  ...
  // --------------------------
  // ACCESS-KEY endpoints (for third-party integrations using API_KEY)
  // --------------------------
  {
    name: 'Access-Key • Find',
    method: 'POST',
    url: '/api/access-key/email/findEmail',
    description: 'Find a single email using an API key (Authorization: Bearer <API_KEY>)',
    headers: [
      { id: '1', key: 'Content-Type', value: 'application/json', enabled: true },
      { id: '2', key: 'Authorization', value: 'Bearer API_KEY_HERE', enabled: true }
    ],
    body: JSON.stringify({ first_name: 'John', last_name: 'Doe', company: 'google.com' }, null, 2)
  },
  {
    name: 'Access-Key • Find Bulk',
    method: 'POST',
    url: '/api/access-key/email/findBulkEmail',
    description: 'Find emails in bulk using an API key (accepts array of { first_name, last_name, domain })',
    headers: [
      { id: '1', key: 'Content-Type', value: 'application/json', enabled: true },
      { id: '2', key: 'Authorization', value: 'Bearer API_KEY_HERE', enabled: true }
    ],
  ...
  {
    name: 'Access-Key • Verify Bulk',
    method: 'POST',
    url: '/api/access-key/email/verifyBulkEmail',
    description: 'Verify many emails at once using an API key',
    headers: [
      { id: '1', key: 'Content-Type', value: 'application/json', enabled: true },
      { id: '2', key: 'Authorization', value: 'Bearer API_KEY_HERE', enabled: true }
    ],
    body: JSON.stringify({ emails: ['john.doe@example.com','jane.smith@example.com'] }, null, 2)
  },
  {
    name: 'Access-Key • Verify',
    method: 'POST',
    url: '/api/access-key/email/verifyEmail',
    description: 'Verify a single email using an API key',
    headers: [
      { id: '1', key: 'Content-Type', value: 'application/json', enabled: true },
      { id: '2', key: 'Authorization', value: 'Bearer API_KEY_HERE', enabled: true }
    ],
    body: JSON.stringify({ email: 'john.doe@example.com' }, null, 2)
  },

  // API key management (proxied)
  {
    name: 'API-Key • List (JWT)',
    method: 'GET',
    url: '/api/api-key/getApiKeys',
    description: 'List all API keys (JWT required)',
    headers: [
      { id: '1', key: 'Authorization', value: 'Bearer ACCESS_TOKEN', enabled: true }
    ]
  ...
  {
    name: 'API-Key • Create (JWT)',
    method: 'POST',
    url: '/api/api-key/createApiKey',
    description: 'Create a new API key (JWT required)',
    headers: [
      { id: '1', key: 'Content-Type', value: 'application/json', enabled: true },
      { id: '2', key: 'Authorization', value: 'Bearer ACCESS_TOKEN', enabled: true }
  ...
  {
    name: 'API-Key • Deactivate (JWT)',
    method: 'DELETE',
    url: '/api/api-key/deactivateAPIKey/REPLACE_KEY_ID',
    description: 'Deactivate an API key by ID (JWT required)',
    headers: [
      { id: '1', key: 'Authorization', value: 'Bearer ACCESS_TOKEN', enabled: true }
    ]
...
// -----------------------------
// API_DOCS (examples) - includes access-key examples
// -----------------------------
const API_DOCS: ApiDoc[] = [
  {
    id: 'doc-email-find',
  ...
  },
  {
    ...
    requestBody: { first_name: 'John', last_name: 'Doe', company: 'google.com' },
    success: { success: true, data: { email: 'john.doe@google.com', confidence: 92, status: 'found', catch_all: false, domain: 'google.com', mx: 'mx.google.com', time_exec: 420, user_name: 'john', connections: 3, ver_ops: 1 }, message: 'Email found' },
    error: { error: { message: 'Unauthorized', code: 401 } },
    responseFields: {
      email: "The email address found.",
      status: "found / not_found / unknown.",
      confidence: "Score 0–100.",
      catch_all: "Boolean if domain accepts all addresses.",
      domain: "Domain used for lookup.",
      mx: "Mail server used.",
      time_exec: "Execution time in ms.",
      user_name: "Username portion."
    },
    statusCodes: { "200": "Found", "401": "Invalid API key", "403": "Key inactive", "402": "No credits" }
  },
  {
    id: 'doc-access-find-bulk',
    name: 'Access-Key • Find Bulk',
    method: 'POST',
    url: '/api/access-key/email/findBulkEmail',
    displayUrl: 'https://server.mailsfinder.com/api/access-key/email/findBulkEmail',
    description: 'Find emails in bulk using an API key',
    headers: { 'Authorization': 'Bearer <API_KEY>', 'Content-Type': 'application/json' },
    requestBody: [ { domain: 'example.com', first_name: 'John', last_name: 'Doe' }, { domain: 'example.com', first_name: 'Jane', last_name: 'Smith' } ],
    success: { success: true, data: { results: [ { email: 'john.doe@example.com', confidence: 95, status: 'found', domain: 'example.com', first_name: 'John', last_name: 'Doe' }, { email: null, confidence: 0, status: 'not_found', domain: 'example.com', first_name: 'Jane', last_name: 'Smith' } ], totalCredits: 2 } },
    error: { error: { message: 'Unauthorized', code: 401 } },
    ...
    statusCodes: { "200": "All processed", "401": "Invalid API key", "429": "Rate limit" }
  },
  {
    id: 'doc-access-verify',
    name: 'Access-Key • Verify',
    method: 'POST',
    url: '/api/access-key/email/verifyEmail',
    ...
    error: { error: { message: 'email is required', code: 400 } },
    responseFields: {
      email: "Email being checked.",
      status: "valid / invalid / unknown.",
      confidence: "Score indicating certainty.",
      deliverable: "Whether mailbox accepts messages.",
      reason: "SMTP server message.",
      catch_all: "Domain catch-all flag."
    },
    statusCodes: { "200": "Verified", "401": "Invalid API key", "402": "No credits", "429": "Rate limit exceeded" }
  },
  {
    id: 'doc-access-verify-bulk',
    name: 'Access-Key • Verify Bulk',
    method: 'POST',
    url: '/api/access-key/email/verifyBulkEmail',
    displayUrl: 'https://server.mailsfinder.com/api/access-key/email/verifyBulkEmail',
    description: 'Verify many emails using an API key',
    headers: { 'Authorization': 'Bearer <API_KEY>', 'Content-Type': 'application/json' },
    requestBody: { emails: ['john.doe@example.com','jane.smith@example.com'] },
    success: { success: true, data: { results: [ { email: 'john.doe@example.com', status: 'valid', confidence: 90, deliverable: true }, { email: 'jane.smith@example.com', status: 'invalid', confidence: 0, deliverable: false } ], totalCredits: 2 } },
    error: { error: { message: 'Unauthorized', code: 401 } },
    responseFields: {
      results: "Array of verification results.",
      totalCredits: "Credits used in the request."
    },
    statusCodes: { "200": "Checked", "401": "Invalid API key", "429": "Rate limited" }
  }
]
...
export default function ApiCallsPage() {
  ...
  // NEW: plaintext key shown once modal
  const [newPlainKey, setNewPlainKey] = useState<string | null>(null)

  // NEW: testing API key for injecting into requests
  ...
  const fetchApiKeys = useCallback(async () => {
    setKeysLoading(true)
    try {
      const res = await apiGet<unknown>('/api/api-key/getApiKeys', { includeAuth: true, useProxy: true })
      if (!res.ok) {
        const msg = typeof res.error === 'string' ? res.error : (res.error && typeof res.error === 'object' && 'message' in res.error ? String((res.error as Record<string, unknown>).message) : 'Failed to fetch API keys')
        toast.error(msg)
        setApiKeys([])
      ...
      const rawList = unwrapData<unknown[]>(res.data) || []
      const list = (Array.isArray(rawList) ? rawList : []).map((item: any) => ({
        id: item.id ?? item._id ?? item._id_str ?? crypto.randomUUID(),
        key_name: item.keyName ?? item.key_name ?? item.key_name_display,
        api_key: item.apiKey ?? item.api_key ?? undefined,         // plaintext only expected on creation
        key_prefix: item.keyPrefix ?? item.key_prefix ?? item.key_prefix_display,
        is_active: item.isActive ?? item.is_active ?? false,
        rate_limit_per_minute: item.rateLimitPerMinute ?? item.rate_limit_per_minute,
        usage_count: item.usageCount ?? item.usage_count ?? 0,
  ...
  const handleCreateKey = async () => {
    ...
    setCreatingKey(true)
    try {
      // POST proxied through same-origin - ensure /api/... goes through Nginx or Next proxy
      const res = await apiPost<unknown>('/api/api-key/createApiKey', { keyName: name }, { includeAuth: true, useProxy: true })
      if (!res.ok) {
        const msg = typeof res.error === 'string' ? res.error : (res.error && typeof res.error === 'object' && 'message' in res.error ? String((res.error as Record<string, unknown>).message) : 'Failed to create API key')
        toast.error(msg)
        return
      ...
      const root = res.data as Record<string, unknown>
      const message = typeof root?.message === 'string' ? root.message : 'API key created'
      // parse data/apiKey reliably
      const createdData = (root?.data ?? root) as Record<string, unknown> | undefined
      const plain = typeof createdData?.['apiKey'] === 'string' ? createdData['apiKey'] as string : undefined

      toast.success(message)
      setNewKeyName('')
      // If server returned plaintext apiKey, show it once in modal
      if (plain) {
        setNewPlainKey(plain)
        // Optionally auto-copy:
        // try { navigator.clipboard.writeText(plain); toast.success('Key copied to clipboard — store it securely') } catch {}
      }
      await fetchApiKeys()
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'Failed to create API key'
  ...
  const handleDeactivate = async (id: string) => {
    setDeactivatingId(id)
    try {
      const res = await apiDelete<unknown>(`/api/api-key/deactivateAPIKey/${id}`, { includeAuth: true, useProxy: true })
      if (!res.ok) {
        const msg = typeof res.error === 'string' ? res.error : (res.error && typeof res.error === 'object' && 'message' in res.error ? String((res.error as Record<string, unknown>).message) : 'Failed to deactivate API key')
        toast.error(msg)
        return
  ...
  const sendRequest = async () => {
    ...
    try {
      // Prepare headers from request builder
      const enabledHeaders = state.currentRequest.headers
        .filter(h => h.enabled && h.key.trim())
        .reduce((acc, h) => ({ ...acc, [h.key]: h.value }), {}) as Record<string,string>

      // If testing API key is provided, set Authorization header (for quick testing only)
      if (testingApiKey && testingApiKey.trim()) {
        enabledHeaders['Authorization'] = `Bearer ${testingApiKey.trim()}`
      }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6 relative">
      ...
      <div className={restricted ? 'pointer-events-none blur-sm' : ''}>
        ...
        {/* NEW: Plaintext API Key modal shown once */}
        {newPlainKey && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white rounded shadow-lg w-full max-w-lg p-6">
              <h3 className="text-lg font-semibold">API Key created — copy it now</h3>
              <p className="text-sm text-muted-foreground mt-1">This key will NOT be shown again. Store it securely.</p>

              <input
                readOnly
                value={newPlainKey}
                className="mt-4 w-full p-2 rounded border font-mono"
                onFocus={(e) => e.currentTarget.select()}
              />

              <div className="mt-4 flex gap-2">
                <Button onClick={() => { navigator.clipboard.writeText(newPlainKey); toast.success('Copied') }}>
                  <Copy className="h-4 w-4 mr-2" /> Copy
                </Button>
                <a
                  className="inline-flex items-center px-3 py-2 rounded border"
                  href={`data:text/plain;charset=utf-8,${encodeURIComponent(newPlainKey)}`}
                  download="mailsfinder_api_key.txt"
                >
                  Download
                </a>
                <Button variant="outline" onClick={() => setNewPlainKey(null)}>I copied it</Button>
              </div>

              <p className="mt-3 text-xs text-red-600">Important: this key will not be retrievable later. If you lose it rotate the key.</p>
            </div>
          </div>
        )}
            ...
            <TabsContent value="request" className="space-y-4">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* NEW: Testing API Key input (for quick manual testing) */}
                  <div className="space-y-2">
                    <Label className="text-sm font-medium">Use API Key (for testing external API)</Label>
                    <div className="flex gap-2">
                      <Input placeholder="Paste API key here (optional for testing)" value={testingApiKey} onChange={(e) => setTestingApiKey(e.target.value)} />
                      <Button variant="outline" onClick={() => { if (testingApiKey) navigator.clipboard.writeText(testingApiKey).then(() => toast.success('Copied')) }}>
                        <Copy className="h-4 w-4 mr-2" /> Copy
                      </Button>
                    </div>
                    <p className="text-xs text-muted-foreground">When set, this key will be added as Authorization header for this request (testing only). Do not store production secrets in browser storage.</p>
                  </div>
