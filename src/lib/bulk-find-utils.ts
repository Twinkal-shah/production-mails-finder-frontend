...
export async function bulkFind(
  ...
  let bodyObj: Record<string, unknown> = typeof body === 'object' && body !== null ? (body as Record<string, unknown>) : {}
  let bodySuccess = typeof bodyObj['success'] === 'boolean' ? (bodyObj['success'] as boolean) : undefined
  ...
  if (!resp.ok || bodySuccess === false) {
    ...
    if (status === 401 || bodyJwtError === true || bodyMessage === 'unauthorized') {
      ...
      if (refreshed) {
        ...
        bodyObj = typeof body === 'object' && body !== null ? (body as Record<string, unknown>) : {}
        bodySuccess = typeof bodyObj['success'] === 'boolean' ? (bodyObj['success'] as boolean) : undefined
      }
    }
    if (!resp.ok || bodySuccess === false) {
      const bodyError = typeof bodyObj['error'] === 'string' ? (bodyObj['error'] as string) : undefined
      const msg = bodyMessage || bodyError || `Request failed (${resp.status})`
      console.error('bulkFind error:', msg)
